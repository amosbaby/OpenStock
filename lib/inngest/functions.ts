    ...
    await step.run("send-welcome-email", async () => {
      try {
        ...
        const introText =
          ...
          "Thanks for joining Openstock. You now have the tools to track markets and make smarter moves.";
    ...
    await step.run("send-news-emails", async () => {
      const results = await Promise.allSettled(
        userNewsSummaries.map(async ({ user, newsContent }) => {
          if (!newsContent) {
            console.log(
              `⏭️ Skipping email for ${user.email} - no news content`
            );
            return false;
          }
          ...
          try {
            console.log(
              `📧 Attempting to send news summary email to: ${user.email}`
            );
            ...
            console.error(
              `❌ Failed to send news summary email to ${user.email}:`,
              error
            );
            throw error;
          }
